...
ENV["SDM_DANGER_BIG_PR_LINES"] = 75

# set the files to watch and warn about if there are changes made
ENV["SDM_DANGER_BUILD_FILES"] = ["Makefile", "Gemfile", "Dangerfile", "circle.yml", ".codeclimate.yml", "tox.ini", "pylintrc"]

# set the files to watch and warn about if there are 
ENV["SDM_DANGER_INSTALL_REQUIREMENTS_FILES"] = ["requirements.txt", "setup.py"]

# set the files to watch and fail if there are changes
ENV["SDM_DANGER_IMMUTABLE_FILES"] = ["LICENSE", "contributing.md", "contributing/code-of-conduct.md"]

# mark the paths that should be reported as part of the circle ci build artifacts
ENV["SDM_DANGER_REPORT_CIRCLE_CI_ARTIFACTS"] = Array[
  {
